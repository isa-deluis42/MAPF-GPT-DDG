...
| `MAPF-GPT-S3` ([`checkpoints/confusion/`](checkpoints/confusion/)) | Stage 3 confusion-AL fine-tune (`confusion_finetune.pt`) | trained, benchmarked |
...
Reported metrics: **ISR** (individual success rate), **CSR** (coverage success rate — all agents reach goals), **ep_length**, **SoC** (sum of costs), **makespan**, **avg_agents_density**, **runtime**. Per-cell results are `mean ± std` over the 128 maps in each suite (1 map for warehouse). The benchmark numbers below come from [`benchmark.txt`](benchmark.txt). Random / Maze / Warehouse have completed for all four algorithms (`Original`, `Baseline` = S1, `Random` = S2, `Confusion` = S3). MovingAI / Puzzles suites are pending for all algorithms.

**Top-line result.** Both human-fine-tuned-ranker variants (`MAPF-GPT-S2`, `MAPF-GPT-S3`) substantially beat the threshold-gated `MAPF-GPT-original` and the auto-only-classifier-gated `MAPF-GPT-S1` on every cell with headroom. S2 and S3 are **roughly tied with a slight edge to S2** — selection strategy on the upstream ranker matters less downstream than the upstream `human_val` lift suggested. The headline numbers, comparing each variant to the threshold-gated baseline:

| Suite | #Agents | Orig | **S2** | Δ S2−Orig | **S3** | Δ S3−Orig |
|---|---|---|---|---|---|---|
| Random | 32 | 0.62 | 0.88 | **+0.26** | **0.90** | **+0.28** |
| Random | 48 | 0.23 | 0.53 | **+0.30** | **0.63** | **+0.40** |
| Random | 64 | 0.09 | **0.33** | **+0.24** (3.7×) | 0.26 | +0.17 |
| Maze | 24 | 0.42 | **0.70** | **+0.28** | 0.66 | +0.24 |
| Maze | 32 | 0.24 | **0.49** | **+0.25** (2×) | 0.42 | +0.18 |
| Warehouse | 64 | 0.719 | **0.984** | **+0.27** | 0.961 | +0.24 |
| Warehouse | 96 | 0.430 | **0.688** | **+0.26** | 0.664 | +0.23 |
| Warehouse | 128 | 0.055 | **0.164** | **+0.11** (3×) | 0.094 | +0.04 |

S2 wins outright on 6 of these 8 cells (Random 64, Maze 24/32, all four Warehouse rows shown); S3 wins outright on 2 (Random 32 by +0.02, Random 48 by +0.10). The **mean ΔCSR across all completed cells with headroom** is +0.21 for S2 and +0.19 for S3 — both far above S1 (+0.06) and Original (baseline). The 78 randomly-sampled human pairs (Stage 2) translate, downstream, into the **single largest result in this paper**; the 65 entropy-AL pairs (Stage 3) approximately replicate the result with smaller label budget but do not improve on it.

**SoC pattern.** `MAPF-GPT-S2` keeps SoC within ≈3% of `MAPF-GPT-original`'s — Pareto dominant, no tradeoff. `MAPF-GPT-S3` consistently pays a small-to-moderate SoC penalty for its CSR gain (Warehouse 64: +16% SoC over Original; Random 32: +1.4%; Maze 32: +1.5%). So S2 is the strictly-better deployment choice; S3 trades a small SoC penalty for slightly different CSR profile.
...
#### 6.7.3 `MAPF-GPT-S3` vs `MAPF-GPT-S2` (confusion-AL ranker → DDG)

The Stage-3-classifier-gated DDG run (`confusion_finetune.pt` as the gating ranker) has now been trained and benchmarked. Upstream (§6.4), Stage 3 beat Stage 2 by +0.014 on `human_val` — a small but reproducible margin. The headline question for §6.7.3 was whether that upstream lift propagates downstream. Two outcomes were equally plausible *a priori*:

1. **Selection strategy matters downstream too.** `MAPF-GPT-S3` beats `MAPF-GPT-S2` by some margin.
2. **Any human supervision suffices downstream.** `MAPF-GPT-S3` ≈ `MAPF-GPT-S2`; the downstream lift comes from the *presence* of human supervision in the upstream ranker, not from the *selection strategy* by which it was collected.

**The data supports outcome 2.** Both variants beat Original by 18-40 pp at moderate density across all three suites, but `MAPF-GPT-S3` does **not** systematically beat `MAPF-GPT-S2` — in fact S2 narrowly wins on 6 of 8 informative cells:

| Suite | #Agents | Original | S1 | **S2** | **S3** | Δ S3−S2 |
|---|---|---|---|---|---|---|
| Random | 32 | 0.62 ± 0.08 | 0.73 ± 0.08 | 0.88 ± 0.06 | **0.90 ± 0.05** | **+0.02** |
| Random | 48 | 0.23 ± 0.07 | 0.39 ± 0.09 | 0.53 ± 0.09 | **0.63 ± 0.09** | **+0.10** |
| Random | 64 | 0.09 ± 0.05 | 0.13 ± 0.06 | **0.33 ± 0.08** | 0.26 ± 0.07 | **−0.07** |
| Maze | 16 | 0.72 ± 0.08 | 0.67 ± 0.08 | 0.86 ± 0.06 | **0.87 ± 0.06** | +0.01 |
| Maze | 24 | 0.42 ± 0.09 | 0.51 ± 0.08 | **0.70 ± 0.07** | 0.66 ± 0.08 | −0.04 |
| Maze | 32 | 0.24 ± 0.07 | 0.22 ± 0.07 | **0.49 ± 0.09** | 0.42 ± 0.09 | −0.07 |
| Warehouse | 64 | 0.719 ± 0.074 | 0.898 ± 0.051 | **0.984 ± 0.020** | 0.961 ± 0.035 | −0.023 |
| Warehouse | 96 | 0.430 ± 0.090 | 0.414 ± 0.082 | **0.688 ± 0.078** | 0.664 ± 0.086 | −0.024 |
| Warehouse | 128 | 0.055 ± 0.039 | 0.047 ± 0.035 | **0.164 ± 0.062** | 0.094 ± 0.051 | **−0.070** |

S2 ≥ S3 on 7 of 9 informative cells (the two ties / wins for S3 are Random 32 by +0.02 and Random 48 by +0.10). S2 wins narrowly on Maze (−0.04 to −0.07) and Warehouse (−0.02 to −0.07). The one cell where S3 clearly outperforms is Random 48 (+0.10 pp); on Random 64 S3 reverses and loses to S2 by −0.07 pp. Across-the-board: mean ΔCSR across all informative cells is +0.21 for S2 and +0.19 for S3 — within the per-cell noise band (≈ ± 1 pp uncertainty per CSR).

**SoC comparison.** S3 pays a small-but-consistent SoC penalty vs S2:

| Suite | #Agents | S2 SoC | S3 SoC | Δ S3−S2 |
|---|---|---|---|---|
| Random | 32 | 1077.8 | 1133.4 | +5.2% |
| Random | 48 | 2636.7 | 2643.9 | +0.3% |
| Maze | 32 | 1931.2 | 1992.4 | +3.2% |
| Warehouse | 64 | 2761.8 | 3119.1 | **+12.9%** |
| Warehouse | 96 | 5337.1 | 6083.2 | +14.0% |

`MAPF-GPT-S2` is therefore the **strictly Pareto-dominant deployment choice**: comparable-or-better CSR than S3 and consistently lower SoC. S3 still beats Original by 18-40 pp CSR — so confusion AL is a clear win over no human labels, just not over random sampling.

**Interpretation.** The upstream `human_val` ordering (S3 > S2 by +0.014) does *not* translate downstream. Two plausible hypotheses for why:

1. **Random sampling has higher pair-diversity than entropy AL.** Confusion AL by construction concentrates queries on borderline pairs (high `H(σ(s_A − s_B))`). Random sampling samples uniformly across the entire pair-pool, including high-confidence pairs the model already gets right. Those "easy" pairs may carry information the model needs at *deployment time* (when DDG's curriculum-shaping must rank both easy and hard rollouts), even though they are wasted at upstream-`human_val` time.
2. **Confusion AL inherits the prior model's biases.** The acquisition function picks pairs where the *Stage-1 baseline* is uncertain — biasing the query set toward the kinds of pairs Stage 1 was already confused about, rather than toward independent coverage of the rollout distribution. Random sampling has no such bias.

Both interpretations have the same actionable implication: **at this label budget, the cheaper random-sampling protocol is at least as good as entropy AL for the downstream-DDG use case.** This is a deployment-cost win.

#### 6.7.4 Is Human Feedback Useful? The S1-vs-S2/S3 Isolation

The three-way upstream split (Stage 1 = no human labels; Stage 2 = 78 randomly-sampled human pairs; Stage 3 = 65 confusion-AL human pairs) was specifically designed to isolate **the value of human supervision** as a separate question from the value of the selection strategy used to collect it. Comparing `MAPF-GPT-S1` (no human pairs) to `MAPF-GPT-S2` / `MAPF-GPT-S3` (with human pairs) gives a clean downstream answer:

| Suite | #Agents | Original | **S1 (no human)** | **S2 (+78 human)** | **S3 (+65 human)** | **Human-feedback effect (S2 − S1)** |
|---|---|---|---|---|---|---|
| Random | 32 | 0.62 | 0.73 | 0.88 | 0.90 | **+0.15** |
| Random | 48 | 0.23 | 0.39 | 0.53 | 0.63 | **+0.14** |
| Random | 64 | 0.09 | 0.13 | 0.33 | 0.26 | **+0.20** |
| Maze | 16 | 0.72 | 0.67 | 0.86 | 0.87 | **+0.19** |
| Maze | 24 | 0.42 | 0.51 | 0.70 | 0.66 | **+0.19** |
| Maze | 32 | 0.24 | 0.22 | 0.49 | 0.42 | **+0.27** |
| Warehouse | 64 | 0.719 | 0.898 | 0.984 | 0.961 | +0.086 |
| Warehouse | 96 | 0.430 | 0.414 | 0.688 | 0.664 | **+0.274** |
| Warehouse | 128 | 0.055 | 0.047 | 0.164 | 0.094 | +0.117 |

The "human-feedback effect" column — the per-cell difference between `MAPF-GPT-S2` (with human pairs) and `MAPF-GPT-S1` (without) — is **+0.086 to +0.274 across nine independent cells**, with a mean of +0.17. This is the cleanest measurement in the paper of what human supervision is worth, and the answer is unambiguous: a small budget of human pairs (78) on the upstream ranker produces a 9-27 pp downstream CSR lift on top of what the auto-only ranker achieves.

The Maze suite is especially diagnostic. At Maze 32 agents, `MAPF-GPT-S1` (auto-only) gets CSR = 0.22 — *worse than the threshold-gated `MAPF-GPT-original` at 0.24*. Adding 78 human pairs (S2) jumps CSR to 0.49 (**2× the threshold-gated original**). The 78 human pairs are not polishing a working ranker; they are *fixing a regime where the auto-only ranker was actively harmful*.

Two summary findings:

- **Human feedback: very useful.** +14-27 pp downstream CSR over the no-human auto-only ranker, at the 65-78-pair label budget. Especially valuable on Maze, where the auto-only ranker actually shaped a *worse* DDG curriculum than the hand-tuned threshold.
- **Selection strategy on the feedback: not useful at this budget.** S2 (random sampling, 78 pairs) ≈ S3 (entropy AL, 65 pairs) downstream, with S2 narrowly winning on 6 of 8 informative cells with smaller SoC (§6.7.3). The cheaper random-sampling protocol is the deployment recommendation.

#### 6.7.5 MovingAI and Puzzles suites [pending]

The MovingAI and Puzzles configurations under `eval_configs/04-movingai/` and `eval_configs/05-puzzles/` have not been benchmarked yet for any of the four algorithms. We expect MovingAI (single large maps with up to 256 agents) to push all methods into the failing regime; Puzzles (5×5 with 2-4 agents) into saturation. The interesting headroom band, based on the Random / Maze / Warehouse results above, is moderate-to-high agent density on geometrically regular maps. [FILL when complete.]
...
**Upstream.** Confusion AL (Stage 3) and random sampling (Stage 2) differ on `human_val` by +0.014 pp — a small but reproducible margin at smaller label budget (65 vs 78). The interpretation we give in §6 is that the right measure is *stable late-epoch* `human_val`, not the best-ever single epoch: random sampling stabilises around 0.66, confusion AL stabilises around 0.71. The +0.05 stable-mean difference is meaningful at a 76-pair signal where each pair is worth ≈ 1.3 pp.

**Downstream (the more important comparison).** The upstream advantage does not transfer: `MAPF-GPT-S2` narrowly beats `MAPF-GPT-S3` on 6 of 8 informative POGEMA cells (§6.7.3). The single largest cell-level S3-over-S2 gap is +0.10 (Random 48); the single largest S2-over-S3 gap is +0.07 (Random 64, Maze 32, Warehouse 128). Mean ΔCSR is +0.21 for S2 vs +0.19 for S3 across all completed cells with headroom. At this label budget, on this DDG-curriculum-shaping use case, **selection strategy on the upstream ranker is essentially neutral downstream** — and if anything, random sampling has a small edge. §7.3a discusses why.

The key methodological caveat applies to both metrics: the `human_val` signal is small enough (76 pairs) that ±5 pp swings on a single re-seed are plausible, and the per-cell POGEMA CSR uncertainty is ≈ ± 1 pp. Multi-seed averaging is on the immediate to-do list, but the directional finding (S2 ≈ S3 downstream despite Stage 3 > Stage 2 upstream) is robust across nine independent cells in three different POGEMA suites.

### 7.3a Upstream ≠ Downstream: Upstream Metrics Are Noisy Predictors

The full S1 / S2 / S3 comparison reveals a counterintuitive pattern: upstream `human_val` ordering does not reliably predict downstream CSR ordering.

| Transition | Δ `human_val` (upstream, §6.2-6.4) | Δ mean CSR (downstream, §6.7) |
|---|---|---|
| S1 → S2 (auto-only → random fine-tune) | +0.05 | **+0.15** (large amplification) |
| S2 → S3 (random fine-tune → confusion AL fine-tune) | +0.014 | **−0.02** (no amplification, slight reversal) |

The S1 → S2 jump amplifies a +0.05 upstream lift into a +14-27 pp downstream lift on multiple cells (Random 32: +0.15, Random 48: +0.14, Random 64: +0.20, etc.). The S2 → S3 jump moves in the opposite direction: a +0.014 upstream lift turns into a roughly tied downstream comparison with S2 narrowly ahead on most cells.

Two non-mutually-exclusive explanations for the asymmetry:

1. **Curriculum compounding is not monotone in upstream quality.** Over 1,500 fine-tune steps, DDG picks 1,500 expert-relabelling targets gated by the classifier. A *different* (not necessarily *better*) ranker shapes a *different* curriculum, and the resulting policy quality depends on the curriculum's *distribution*, not just the ranker's average accuracy. The S2 and S3 rankers may have similar `human_val` but produce systematically different curricula, with the S2 curriculum happening to be the one MAPF-GPT learns from best.
2. **Diversity > uncertainty for curriculum shaping.** Confusion AL by construction concentrates its 65 queries on borderline pairs the Stage-1 baseline is most uncertain about — high-entropy queries. Random sampling spreads its 78 queries across the entire pair-pool, including pairs the model already handles well. The "wasted" easy-pair labels in random sampling appear to teach the *classifier* something at deployment time (when the curriculum needs to rank both easy and hard rollouts) that the concentrated entropy-AL labels do not.

**Actionable implication:** at this label budget, upstream `human_val` is a poor proxy for downstream curriculum quality. Future deployments should treat the upstream classifier metric as a sanity check ("does the classifier rank human-clearly-worse-as-worse?") rather than as a deployment-target metric. The real deployment target is the downstream MAPF-GPT policy quality, which must be measured directly. A corollary: the cheaper random-sampling annotation protocol is at least as good as entropy AL for the downstream-DDG use case, which is a deployment-cost win.

### 7.3b The Maze Diagnostic

The Maze suite is where the human-supervision story is cleanest. On Maze, `MAPF-GPT-S1` (auto-only ranker) was *losing* to `MAPF-GPT-original` at 4 of 6 densities — the auto-only ranker actually shaped a *worse* DDG curriculum than the hand-tuned threshold for maze-topology rollouts. Both human-fine-tuned variants reverse this: `MAPF-GPT-S2` (random-sampled 78 human pairs) beats Original at every Maze density, with the largest gain at 24-32 agents (+25-28 pp); `MAPF-GPT-S3` (confusion-AL 65 human pairs) also reverses Original, with slightly smaller gains (+18-24 pp at 24-32 agents). Either flavor of human supervision *fixes a regime where the auto-only ranker was actively harmful* — direct evidence that the human signal teaches the ranker something about congestion in maze topology that LaCAM-diff alone cannot pick up. The S2 > S3 ordering at this scale is small (+4-7 pp on Maze 24/32) and within the per-cell ±1 pp noise — at this label budget, the Maze diagnostic does not separate the two human-supervision protocols meaningfully.
...
Within that budget, the three-stage comparison cleanly answers three HRI-design questions, and the answers diverge depending on whether you measure the upstream classifier or the downstream MAPF-GPT policy.

**Q1: Does fine-tuning on a small budget of human pairs damage the auto-aligned ranker?** No. `pair_acc` is preserved to within 0.002 across all three stages (§6.2-6.4). Phase-1+Phase-2 `--human-only` training cleanly separates "learn auto-pair ranking" from "perturb toward human judgment."

**Q2: Is human feedback useful for the downstream MAPF-GPT policy?** Yes, substantially. Both human-fine-tuned-classifier-gated variants (`MAPF-GPT-S2`, `MAPF-GPT-S3`) deliver +14-27 pp downstream CSR over the no-human-feedback `MAPF-GPT-S1` across nine cells in three POGEMA suites (§6.7.4 isolation table). On Maze, the auto-only ranker was actively *worse* than the hand-tuned threshold at 4 of 6 densities; human supervision fixes that regime and converts it into a +25-28 pp win. The qualitative HRI claim — "human judgment carries information the auto-label cannot recover" — is empirically borne out by a downstream-policy measurement that does not depend on any subjective held-out human-pair signal.

**Q3: Does an entropy-driven acquisition unlock more value than uniform random sampling?** Yes upstream (modestly: +0.014 on `human_val`), no downstream. `MAPF-GPT-S3` (confusion AL) and `MAPF-GPT-S2` (random sampling) are roughly tied on the POGEMA suites with S2 narrowly winning on 6 of 8 informative cells with smaller SoC (§6.7.3). At this label budget, **the cheaper random-sampling annotation protocol is at least as good as entropy-driven AL for the downstream-DDG deployment use case** — a deployment-cost win and a counterpoint to the upstream-only AL literature.

The combined practical recommendation for an HRI loop that wants to add a small budget of human supervision to a DDG-style data-curation pipeline: (i) use a two-phase `--human-only` fine-tune so the human gradient is not drowned by auto pairs; (ii) collect labels via uniform random sampling rather than entropy-driven AL (cheaper, equally effective downstream); (iii) measure success directly on the downstream task (CSR), not on a held-out human-pair classifier metric.
...
- **Closed-loop slide (the headline).** Replacing DDG's hand-tuned diff threshold with a **human-fine-tuned segment classifier**, then retraining MAPF-GPT under that classifier-gated DDG, dominates the original on every cell with headroom: Warehouse-64 CSR jumps from 0.72 to **0.98** (S2) / **0.96** (S3); Random-48 from 0.23 to **0.53** (S2) / **0.63** (S3); Maze-32 from 0.24 to **0.49** (S2) / **0.42** (S3). The auto-only ranker (S1) gets only +11/+16/+18 pp on Random/Warehouse and ties on Maze — isolating the value of the human pairs. Numbers from [`benchmark.txt`](benchmark.txt), `ckpt_ddg_1500.pt` cut. Disambiguate eval-config keys: `Baseline` = S1, `Random` = S2, `Confusion` = S3.
- **The S2 ≈ S3 surprise.** Upstream, Stage 3 (confusion AL) beats Stage 2 (random sampling) on `human_val` by +0.014. Downstream, `MAPF-GPT-S3` does **not** beat `MAPF-GPT-S2` — S2 wins 6 of 8 informative cells, with consistently lower SoC. The S1 → S2 transition amplified +0.05 upstream into +14-27 pp downstream; the S2 → S3 transition shrank +0.014 upstream into a small downstream reversal. **Upstream `human_val` is a noisy predictor of downstream CSR.** The deployment-cost win: random sampling is at least as good as entropy AL.
- **Maze diagnostic slide.** On Maze, `MAPF-GPT-S1` (auto-only) was *losing* to Original at 4 of 6 densities. Both `MAPF-GPT-S2` and `MAPF-GPT-S3` reverse this: S2 beats Original by +25-28 pp at 24-32 agents; S3 beats by +18-24 pp. The 78 / 65 human pairs fix a regime where the auto-only ranker was actively harmful — direct evidence the human signal teaches the ranker something about maze-topology congestion that LaCAM-diff alone cannot capture.
